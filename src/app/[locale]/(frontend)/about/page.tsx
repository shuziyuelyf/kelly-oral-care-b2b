...
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white pt-20">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, i) => (
              <div key={i} className="text-center p-6 bg-white rounded-lg">
                <stat.icon className="w-8 h-8 text-[#E8720C] mx-auto mb-3" />
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-[#1B3A5C] text-center mb-10">{t('history')}</h2>
              ...
              <div key={i} className={`flex items-center gap-8 mb-8 ${i % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
                <div className={`flex-1 ${i % 2 === 0 ? 'text-right' : 'text-left'}`}>
                  <div className="inline-block bg-white p-4 rounded-lg shadow-sm border">
                    <div className="text-[#E8720C] font-bold text-lg">{item.year}</div>
                    <div className="text-gray-600 text-sm mt-1">{item.event}</div>
                  </div>
                </div>
                <div className="w-4 h-4 bg-[#E8720C] rounded-full border-4 border-white shadow z-10" />
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-[#1B3A5C] text-center mb-10">{t('honors')}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {honors.map((honor, i) => (
              <div key={i} className="bg-white p-6 rounded-lg text-center shadow-sm">
                <Award className="w-8 h-8 text-[#E8720C] mx-auto mb-3" />
                <p className="font-medium text-[#1B3A5C] text-sm">{honor.title}</p>
              </div>
            ))}
          </div>
        </div>
