  ...
  return (
    <div className="min-h-screen bg-white pt-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <article>
          <h1 className="text-3xl font-bold text-[#173A63] mb-4">{title}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {relatedNews.map((rn) => (
                <Link key={rn.id} href={`/${locale}/news/${rn.id}`} className="group bg-gray-50 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-3">
                    <h3 className="text-sm font-medium text-[#173A63] group-hover:text-[#008FD5] line-clamp-2">{getI18nValue(rn.i18n, lang, 'title')}</h3>
