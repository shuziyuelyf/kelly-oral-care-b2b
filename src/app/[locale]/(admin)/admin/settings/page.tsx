        ...
        {sections.map((section, i) => (
          <div key={i} className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-[#173A63]/10 rounded-lg flex items-center justify-center">
                <section.icon className="w-5 h-5 text-[#173A63]" />
              </div>
              <div>
                <h3 className="font-semibold text-[#173A63]">{section.title}</h3>
                <p className="text-sm text-gray-500">{section.desc}</p>
              </div>
            </div>
            <ul className="space-y-2">
              {section.items.map((item, j) => (
                <li key={j} className="text-sm text-gray-600 hover:text-[#008FD5] cursor-pointer py-1 border-b border-gray-50 last:border-0">{item}</li>
              ))}
            </ul>
          </div>
        ))}
